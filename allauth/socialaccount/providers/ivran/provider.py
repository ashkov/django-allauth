...
class IvranAccount(ProviderAccount):
    def to_str(self):
        first_name = self.account.extra_data.get("first_name", "")
        last_name = self.account.extra_data.get("last_name", "")
        name = " ".join([first_name, last_name]).strip()
        return name or super(IvranAccount, self).to_str()


class IvranProvider(OAuth2Provider):
    id = "ivran"
    name = "Ivran"
    account_class = IvranAccount
...
provider_classes = [IvranProvider]
